...
import { MyraaDevice } from './MyraaDevice';
...
export class SystemControlManager {
  ...
  public async torch_on(): Promise<SystemControlResult> {
    ...
    // REAL native Android flashlight
    if (MyraaDevice.isNativeAndroid()) {
      const result = await MyraaDevice.turnTorchOn();

      if (result.success) {
        this.isTorchActive = result.enabled;
        this.notifyStateChange();

        return {
          success: true,
          action: 'torch_on',
          current_state: result.enabled,
      ...
      return {
        ...
        action: 'torch_on',
        current_state: false,
        message: 'Could not turn on the Android flashlight.',
        error: 'NATIVE_TORCH_FAILED',
        executionTimeMs: Math.round(performance.now() - start),
      };
    }
    ...
    try {
      ...
      const track =
        this.torchMediaStream.getVideoTracks()[0];
      ...
      return {
        ...
        current_state: false,
        message:
          'Flashlight hardware is not available on this device camera.',
      ...
      return {
        ...
        action: 'torch_on',
        current_state: false,
        message: `Could not turn on torch: ${
          err.message || 'Permission denied or unsupported'
        }`,
        error: err.message,
        executionTimeMs: Math.round(
          performance.now() - start
  ...
  public async torch_off(): Promise<SystemControlResult> {
    const start = performance.now();

    // REAL native Android flashlight
    if (MyraaDevice.isNativeAndroid()) {
      const result = await MyraaDevice.turnTorchOff();
      ...
      return {
        ...
        message: result.success
          ? 'Torch turned off.'
          : 'Could not turn off the Android flashlight.',
  ...
  public async torch_toggle(): Promise<SystemControlResult> {
    ...
    if (MyraaDevice.isNativeAndroid()) {
      ...
      return {
        ...
        message: result.success
          ? result.enabled
            ? 'Torch turned on successfully.'
            : 'Torch turned off.'
          : 'Could not toggle the Android flashlight.',
        error: result.success
          ? undefined
          : 'NATIVE_TORCH_FAILED',
        executionTimeMs: Math.round(
